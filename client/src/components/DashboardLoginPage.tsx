 ...
 * Auth: delegates to dashboardLogin() from dashboardAuth.ts
 */
...
import { dashboardLogin, getDashboardSession } from "@/lib/dashboardAuth";
...
export function DashboardLoginPage({ role }: DashboardLoginPageProps) {
  ...
  }, [role, config.dashboardPath, setLocation]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const session = dashboardLogin(role, email, password);
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <Mail
                    size={15}
                    className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#AAAAAA] pointer-events-none"
                  />
                  <input
                    ref={emailRef}
                    id="email"
              ...
              <button
                ...
                style={{ background: loading ? "#888888" : config.color }}
              >
                {loading ? (
                  <>
